    ...
    try {
      ...
      if ('status' in (err as FetchBaseQueryError)) {
        ...
        if ((err as FetchBaseQueryError).status === 400) {
          setErr('Username, email and Password Required!')
        }
      ...
      {isSuccess ? <Success isLogin={false} /> : (<div><div><img className='mx-auto drop-shadow-2xl h-40 fadeScaleBounce' src={Logo} alt="Logo Here" /></div><section className='bg-emerald-400 shadow-2xl max-w-sm  mt-10 py-14 px-10 mx-auto'>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex items-center space-x-2 mt-4">
            ...
            <span className={emailValid ? 'text-green-800' : 'hidden'}>
              <FaCheck className='text-2xl' />
            </span>
            <span className={!email || emailValid ? 'hidden' : 'text-red-600'}>
              <FaTimes className='text-2xl' />
            </span>
          </div>
          <input
          ...
          />
          <p id='uemailnote' className={`${emailFocus && email && !emailValid ? 'text-white bg-black rounded-md p-1 mt-6' : 'hidden'} text-sm mt-1`}>
            <FaInfoCircle />Must contain an "@" symbol separating the username and domain parts.<br />
The username may include alphanumeric characters, dots, underscores, and hyphens.<br />
The domain must consist of letters, digits, hyphens, and dots.<br />
Top-Level Domain (TLD) must be at least 2 alphabetic characters (e.g., .com, .org).<br />
Cannot start or end with special characters such as dots, hyphens, or underscores.<br />
          </p>
